...
> [!NOTE]
> В публичный доступ не передаётся (коммерческая разработка). По запросу для работодателя — краткий разбор архитектуры (хаб ↔ publisher, state, MTProto) без секретов заказчика.

## Лицензия

> [!CAUTION]
> Описание и скриншоты — для портфолио. Права на продукт у заказчика; копирование кода из этого репозитория не предполагается (кода здесь нет).
